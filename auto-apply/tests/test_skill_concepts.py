...
class TestCompoundsSplitOnlyWhenSafe(unittest.TestCase):
    ...
    def test_the_audit_compounds_split(self):
        ...
        # CONTRACT CHANGE, deliberate. Neither "authentication" nor
        # "security" is a concept this table knows, and the canonical
        # hardening batch requires POSITIVE evidence that a compound's
        # parts are independent concepts before splitting. See
        # test_canonical_hardening.TestUnknownCompoundsSurvive.
    ...
    def test_an_unknown_parenthetical_is_kept_rather_than_trimmed(self):
        """CONTRACT CHANGE. The annotation used to be dropped, which also
        dropped half of "AcmeTool (Enterprise)". The whole string is kept
        and resolve() still maps it to the right concept."""
        self.assertEqual(sc.split_compound("Redis (caching)"),
                         ["redis (caching)"])
        self.assertEqual(sc.resolve("Redis (caching)"), "redis")
        self.assertEqual(sc.resolve("Python (scripting)"), "python")
    ...
    def test_an_unknown_compound_no_longer_splits(self):
        """CONTRACT CHANGE. Bun and Deno are real runtimes and neither is
        in the table, so splitting was a guess. The cost is recall on a
        résumé that writes them this way; the benefit is that "Foo/Bar"
        and "Research & Development" stop becoming invented skills."""
